...
PUBLIC_URL  = os.environ.get("RENDER_EXTERNAL_URL", "").strip()  # Render lo setta automaticamente
...
async def main():
    ...
    log.info("PumpScan v26 (Web Service) starting")
    ...
    if PUBLIC_URL:
        log.info("Public URL: %s — keepalive enabled", PUBLIC_URL)

    timeout = aiohttp.ClientTimeout(total=30)
    connector = aiohttp.TCPConnector(limit=30)
    async with aiohttp.ClientSession(timeout=timeout, connector=connector) as session:
        # Avvio server HTTP per Render Web Service (richiede una porta in ascolto)
        app = web.Application()
        ...
        async def status(request):
            return web.json_response({
                ...
                "scans": SCAN_COUNT,
                "notif_sent": NOTIF_SENT_COUNT,
                "sticky": len(trend_sticky),
            })
        ...
        await ntfy_send(session, "🟢 PumpScan v26 attivo (Web Service)",
                        f"Scansione DexScreener Solana ogni {POLL_SEC}s.\n"
                        f"⚡ spike 1h\n🚀 breakout precoce\n🌱 accumulazione\n"
                        f"🟢 trend basso MC\n📈 trend alto MC\n📌 trend ancora attivi\n"
                        f"URL: {PUBLIC_URL or 'localhost'}",
...
# Keepalive: ogni 10 minuti pinga il proprio URL pubblico per evitare lo sleep
# Render Free Web Service si addormenta dopo 15 min di inattività HTTP.
# Tenerlo sveglio con un self-ping ogni 10 min lo mantiene attivo 24/7.
async def keepalive_loop(session):
    if not PUBLIC_URL:
        log.warning("No PUBLIC_URL set, keepalive disabled (set RENDER_EXTERNAL_URL)")
        return
    await asyncio.sleep(60)  # aspetta che il server sia up
    url = PUBLIC_URL.rstrip("/") + "/ping"
    while True:
        try:
            async with session.get(url, timeout=aiohttp.ClientTimeout(total=15)) as r:
                log.info("keepalive ping: %s → %s", url, r.status)
        except Exception as e:
